...
The agent's immediate context window. Holds recent observations, conversation history, and active tool results. Fastest access (&lt;100ms), limited capacity (up to 10,000 entries), and entries are marked as consolidated via sleep consolidation rather than deleted.
...
5. **Mark** the original Working Memory entries as consolidated (sets `consolidated_at`)

Sleep is triggered explicitly — it does not run automatically on a timer.

## MEMORIA (v3.0+)

MEMORIA is a structured fact extraction and retrieval layer added in v3.0. It builds five new SQLite tables alongside the standard memory tiers:

| Table | What it stores | Key columns |
|-------|---------------|-------------|
| `memoria_facts` | Key-value facts (metrics, dates, versions) | key, value, fact_type, version_id |
| `memoria_timelines` | Chronological event index | date, description, session_id |
| `memoria_kg` | Subject-predicate-object triples | subject, predicate, object |
| `memoria_instructions` | Extracted user instructions | instruction, topic, context_snippet, source_memory_id |
| `memoria_preferences` | User preference statements | preference, topic, evolution, context_snippet, source_memory_id |

During `remember()`, MEMORIA regex-mining extracts structured data from content and stores it in these tables. Retrieval routes queries by ability type (IE: fact lookups, TR: timeline searches, CR: negation queries, etc.).

## V3 Features

### Entity Extraction

`remember()` accepts `extract_entities=True` to automatically detect named entities (mentions, hashtags, proper nouns) using regex patterns and Levenshtein fuzzy matching. Extracted entities are stored as AnnotationStore annotations.

### Fact Extraction

`remember()` accepts `extract=True` to send content through an LLM pipeline that extracts 2–5 structured factual statements, stored as AnnotationStore annotations. The extraction pipeline gracefully falls back through remote API → local GGUF → skip.
...
A plugin base class (`MnemosynePlugin`) with 4 lifecycle hooks. Built-in plugins: `LoggingPlugin`, `MetricsPlugin`, `FilterPlugin`, `CompressionPlugin`. Discovers plugins from `~/.hermes/mnemosyne/plugins/`.
