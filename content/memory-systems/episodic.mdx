...
## MEMORIA Structured Tables (v3.0+)

Alongside episodic memory, v3.0 added a parallel structured retrieval layer called MEMORIA. These tables are populated automatically during `remember()` and provide ability-routed fact lookups:

| Table | Stores | Key Columns |
|-------|--------|-------------|
| `memoria_facts` | Key-value metrics, dates, versions | `key`, `value`, `fact_type`, `version_id` |
...
See the MEMORIA architecture page for details on how these tables are queried during recall.
