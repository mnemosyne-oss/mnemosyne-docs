...
Environment="MNEMOSYNE_MCP_TOKEN=***"
ExecStart=/opt/mnemosyne/venv/bin/python -m mnemosyne.mcp_server --transport sse --host 0.0.0.0 --port 8080
Restart=always
RestartSec=5
...
cd /opt/mnemosyne
python3 -m venv venv
venv/bin/pip install "mnemosyne-memory[mcp]"
'
...
<CodeBlock code={`/opt/mnemosyne/data/*.log {
    daily
    rotate 7
    compress
    delaycompress
