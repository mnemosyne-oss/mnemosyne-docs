...
# MCP_TOKEN required when binding 0.0.0.0; set via docker-compose environment
...
services:
  mnemosyne:
    ...
    healthcheck:
      test: ["CMD", "curl", "-f", "-s", "http://localhost:8080/sse"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 10s
...
<CodeBlock code={`curl -s http://localhost:8080/sse
`} language="bash" />
