# Installation

Mnemosyne v2.0.0 is a standalone Python package — no agent framework required. This page walks through every step from prerequisites to verification.

## Prerequisites

Before installing Mnemosyne, make sure you have:

- **Python 3.10 or higher** — check with `python --version`
- **pip** or **uv** package manager available in your shell

## Quick Install

Two commands. That's all it takes.

**Step 1 — Install the package:**

```bash
# Using pip
pip install mnemosyne-memory[embeddings]

# Using uv (faster)
uv pip install mnemosyne-memory[embeddings]
```

**Step 2 — Verify:**

```bash
python -c "import mnemosyne; print(mnemosyne.__version__)"
# Should print: 2.0.0
```

That's it. No database setup, no container orchestration, no config files. Mnemosyne creates its SQLite database automatically on first use.

## Install Options

### pip vs uv

Both work. `uv` is faster, but use the matching command:

| Tool | Install command |
|------|-----------------|
| `pip` | `pip install mnemosyne-memory[embeddings]` |
| `uv` | `uv pip install mnemosyne-memory[embeddings]` |

### Extras

Mnemosyne ships optional dependency groups you can install as needed:

| Extra | Command | What it adds |
|-------|---------|--------------|
| Core (not recommended) | `pip install mnemosyne-memory` | Base package with FTS5 keyword search only |
| **Embeddings** | `pip install mnemosyne-memory[embeddings]` | **+ fastembed for semantic vector search** |
| LLM consolidation | `pip install mnemosyne-memory[llm]` | + ctransformers for local LLM sleep consolidation |
| Everything | `pip install mnemosyne-memory[all]` | All optional dependencies |

## Optional: sqlite-vec for Performance

`sqlite-vec` is an **optional performance boost** for large datasets (100K+ memories). It moves vector similarity search into native C code inside SQLite. Without it, Mnemosyne uses an in-memory numpy fallback that works fine for most use cases.

```bash
pip install sqlite-vec
```

- **With `[embeddings]` only:** Full semantic search + hybrid retrieval via in-memory numpy cosine similarity. Works great for most deployments.
- **With `[embeddings]` + `sqlite-vec`:** Same semantic search, but vector queries run in native C inside SQLite. Faster on very large datasets.
- **Without `[embeddings]`:** FTS5 keyword search only, regardless of sqlite-vec.

Install sqlite-vec only if you need maximum performance on huge datasets.

## Verify Installation

Check the installed version directly:

```bash
python -c "import mnemosyne; print(mnemosyne.__version__)"
```

You can also run a quick smoke test:

```python
from mnemosyne import Mnemosyne

mem = Mnemosyne()
mem.remember("Installation test", importance=0.1)
results = mem.recall("test")
print(f"Stored and retrieved {len(results)} memory")
```

## MCP Server Setup

Mnemosyne v2 includes a built-in MCP server for Claude Desktop and other MCP clients:

```bash
# Add to your Claude Desktop config (claude_desktop_config.json):
{
  "mcpServers": {
    "mnemosyne": {
      "command": "mnemosyne",
      "args": ["mcp"]
    }
  }
}
```

For SSE transport (web clients):

```bash
mnemosyne mcp --transport sse --port 8080
```

## Hermes Agent Integration

Mnemosyne is the native memory backend for [Hermes Agent](https://github.com/NousResearch/hermes-agent).

### One-Liner Deploy (fastest)

No pip, no venv — just a symlink:

```bash
curl -sSL https://raw.githubusercontent.com/AxDSan/mnemosyne/main/deploy_hermes_provider.sh | bash
```

Then configure Hermes:

```bash
hermes config set memory.provider mnemosyne
```

And add to `plugins.enabled` in `~/.hermes/config.yaml`:

```yaml
plugins:
  enabled:
    - mnemosyne
```

Restart and verify:

```bash
hermes gateway restart
hermes memory status
hermes mnemosyne stats
```

### pip + Register

If you prefer the PyPI package:

```bash
pip install mnemosyne-memory[embeddings]
python -m mnemosyne.install
hermes gateway restart
```

### Manual Config

Alternatively, configure manually in `~/.hermes/config.yaml`:

```yaml
memory:
  provider: mnemosyne

plugins:
  enabled:
    - mnemosyne
```

## Development Install (from Source)

If you're contributing or want to run the latest unreleased changes:

```bash
# Clone the repo
git clone https://github.com/axdsan/mnemosyne.git
cd mnemosyne

# Create a virtual environment
python -m venv .venv
source .venv/bin/activate

# Install in editable mode with dev dependencies
pip install -e ".[dev]"
```

The `-e` flag installs in "editable" mode — changes to the source code take effect immediately without re-installing. Perfect for development and debugging.
