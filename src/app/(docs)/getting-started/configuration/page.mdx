...
| `MNEMOSYNE_LLM_MAX_TOKENS` | `256` | Max output tokens for generated summaries |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `MNEMOSYNE_EXTRACTION_PROMPT` | built-in | Custom prompt for fact extraction during `extract=True` |
...
<CodeBlock code={`# config.yaml (Hermes project root)
memory:
  mnemosyne:
