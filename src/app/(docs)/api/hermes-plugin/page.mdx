...
|| Hook | Timing | Action |
||------|--------|--------|
|| `prefetch` (MemoryProvider) | Before LLM call | Inject Mnemosyne context into agent prompt |
|| `on_turn_start` | Turn start | Track turn count for auto-save |
|| `sync_turn` | Turn end | Persist filtered conversation turn to working memory |
|| `on_session_end` | Session end | Run consolidation cycle |
|| `on_memory_write` | Built-in memory write | Mirror Hermes's native memory writes to Mnemosyne |
