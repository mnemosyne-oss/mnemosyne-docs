...
| `channel_id` | `str \| None` | `None` | Channel for cross-session shared memory (v2.1) |
...
| `veracity` | `str` | `"unknown"` | Confidence level: `"stated"`, `"inferred"`, `"tool"`, `"imported"`, `"unknown"` (v2.3) |
...
<CodeBlock code={`degrade_episodic(dry_run: bool = False) -> dict
`} language="python" />
...
<CodeBlock code={`get_contaminated(limit: int = 50, min_importance: float = 0.0) -> list[dict]
`} language="python" />
