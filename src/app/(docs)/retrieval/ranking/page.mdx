...
The three components are combined with **configurable weights**:
...
<Callout type="info" title="Configurable Weights">
The scoring weights (0.5 / 0.3 / 0.2) are per-query configurable through the `recall()` API via `vec_weight`, `fts_weight`, and `importance_weight` parameters. See [Configurable Scoring](/retrieval/configurable-scoring).
</Callout>
