...
Adaptive Associative Abstraction Kernel (AAAK) is Mnemosyne's text substitution system for reducing memory content size. It is **not** a method on the `Mnemosyne` class — it is a standalone utility used during consolidation.
...
1. **Category prefixes**: Replaces verbose category labels with short tokens (e.g., `"PREFERENCE: ..."` → `"PREF|..."`)
