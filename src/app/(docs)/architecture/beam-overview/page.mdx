...
- **Retrieval**: Hybrid (vector + FTS5 + binary_vectors) with **configurable scoring weights** (`vec_weight`, `fts_weight`, `importance_weight`, defaulting to 50/30/20). Binary vectors provide fast pre-filtering via Hamming distance for large datasets.
